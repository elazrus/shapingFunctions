...
public class AdjustableCenterSquircularWindow extends Function {
	private float a;
	private int n;
	...
	public float applyFunction(float x, boolean clamp) {
		...
		float y = 0;
		float pwn = PApplet.max(2.0f, n*2.0f);
		
		if (Float.compare(x, fa) <= 0) {
			y = (1.0f - fa) + PApplet.pow(PApplet.pow(fa, pwn) - PApplet.pow(x-fa, pwn), 1.0f/pwn);
		}
		else {
			y = fa + PApplet.pow(PApplet.pow(1.0f-fa, pwn) - PApplet.pow(x-fa, pwn), 1.0f/pwn);
		}
